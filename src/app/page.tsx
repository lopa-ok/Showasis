...
import { loadLocalIdentity, saveDisplayName } from "@/lib/identity";
...
export default function HomePage() {
  ...
  const handleNameChange = (value: string) => {
    setIdentity((prev: LocalIdentity) => ({ ...prev, displayName: value }));
  ...
  const currentSlot = slots.find((slot: SlotView) => slot.id === currentSlotId);
  const nextSlot = slots.find((slot: SlotView) => slot.id === nextSlotId);
  const canBook = Boolean(identity.displayName.trim());
      ...
      <section className="calendar-layout">
        ...
        <article className="panel panel-solid panel-glow">
          <header className="panel-header">
            ...
            <span className="panel-tag">local only</span>
          </header>
          <p className="panel-body">Enter a display name to reserve or cancel your shower slots.</p>
          <label className="form-field">
            ...
            <input
              ...
              placeholder="e.g. stasis traveler"
              maxLength={48}
            />
          </label>
          <p className="panel-footnote">
            Display names stay on this device until Slack auth arrives.
          </p>
        </article>
        ...
        <article className="panel panel-solid panel-accent">
          <header className="panel-header">
            ...
            <span className="panel-tag">{DateTime.now().setZone(timezone).toFormat("LLL dd")}</span>
          </header>
        ...
        <section className="calendar-board">
                ...
                <div key={day.key} className="calendar-day">
                  ...
                  <ul className="calendar-slots">
                    {day.slots.map((slot) => {
                      ...
                      const canInteract = canBook && slot.type === "booking";
                      return (
                        <li
                          key={slot.id}
          ...
          {!canBook && <p className="panel-footnote">Add a display name to unlock booking controls.</p>}
        </section>
      </section>
    </AppShell>
  );
